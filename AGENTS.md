...
> **Tool-agnostic workflow contract.** Both Claude Code and OpenAI Codex read this file. For tool-specific extensions (model routing, live progress tracker, slash commands, config locations), see:
>
> - `docs/reference/tools/claude-code.md` — Claude Code overlay
> - `docs/reference/tools/codex.md` — Codex overlay
>
> Claude Code additionally reads `CLAUDE.md` at the root, which is a thin pointer to this file + the Claude overlay. When a tool-specific behavior is called for (e.g., *"populate the live progress tracker"*) the core rule lives here; the overlay names the mechanism.
...
3. **Three-tier context.** Tier 1 (always-on, in repo) / Tier 2 (fetch-on-command, cloud mirror + git) / Tier 3 (cloud indexes — `COMPLETED-SLICES.md`). The AI reads Tier 2 deliberately, not automatically. Full model: `docs/ARCHIVE.md`.
4. **Shared lexicon.** Brand vocab + industry vocab + AI-tool vocab + workflow vocab live in `docs/reference/VOCAB.md` (Tier 1). Consult before inventing a new term.
5. **Tool-agnostic by design.** This file describes the workflow. Which AI tool runs it (Claude Code, Codex, or future) is a per-session choice — see the tool overlays in `docs/reference/tools/`.

## Abstract roles (resolved per-tool in overlays)

The workflow names **roles**, not specific tools or models. Each tool's overlay (`docs/reference/tools/<tool>.md`) resolves every role to a concrete mechanism. Swapping a role's resolution is a **one-line edit** in the overlay — sessions, rules, and checklists never change.

| Role                              | What it does                                                                                     |
| --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **deeper-reasoning model**        | Deep design thinking, novel debugging, real tradeoff decisions                                   |
| **deeper-reasoning model (XL)**   | Same, but with an enlarged context window for whole-codebase Q&A                                 |
| **faster/cheaper model**          | Clear-plan execution, research, file-reading, bounded summarization                              |
| **live progress tracker**         | Session-visible task list with `pending`/`in_progress`/`completed` states                        |
| **parallel-dispatch mechanism**   | Spin off bounded independent tasks to concurrent workers, isolated from parent session context   |
| **mid-session model switch**      | Change the session's model without restarting (pays a cache-invalidation tax)                    |
| **context-budget check**          | Show current % of window used, to compare against the 40/65/80 % thresholds                      |

**Swap strategy.** If a newer model outclasses the current binding for a role, edit **one line in the overlay** — not this file.

### Stage → role routing

| Stage                                       | Role                             |
| ------------------------------------------- | -------------------------------- |
| L-slice Planning                            | deeper-reasoning model (XL)      |
| L-slice Implementation                      | deeper-reasoning model           |
| M-slice session (default)                   | faster/cheaper model             |
| S-slice session                             | faster/cheaper model             |
| Closing session                             | faster/cheaper model             |
| Subagent — research / summary / reading     | faster/cheaper model             |
| Subagent — architecture / novel debug       | deeper-reasoning model           |

Stage names are stable; role resolutions are in the overlay.

### Infrastructure (tool-agnostic)

Both Claude Code and Codex support:

- **MCPs** (Model Context Protocol) — standardized tool-augmentation servers. Project MCP definitions live at `.mcp.json` at the repo root (tool-agnostic format). Per-tool enablement mechanisms differ; see overlays.
- **Skills** — reusable task patterns/recipes. Each tool has its own skill registry; some skills are cross-tool. **The workflow never depends on a specific skill existing** — skills are optional enhancements.

### Portability guarantee

Clone this repo on any machine with **either** Claude Code **or** Codex installed (or both) and run the workflow end-to-end:

- `AGENTS.md` — tool-agnostic contract (required, auto-loaded by both tools)
- `docs/reference/tools/claude-code.md` — Claude Code bindings (consulted when Claude Code is the active tool)
- `docs/reference/tools/codex.md` — Codex bindings (consulted when Codex is the active tool)
- `CLAUDE.md` — thin pointer for Claude Code's native auto-load; safe to delete without breaking Codex usage

If only one tool is installed, the other's overlay is simply unread — no functionality breaks. If both are installed, pick per task (see Stage → role routing).
...
- **OS-specific command quirks:** `<cloud>/workflow-knowledge/os-quirks/<os>.md`. Check here first when troubleshooting a platform-specific command. Append new discoveries before closing any slice (hard rule).
...
- **M-slices plan inline** (live progress tracker + 1-paragraph "Plan" at top of `log.md`) — no separate planning session, no `spec.md`, no `plan.md`.
...
3. **Material model downshift** — switching from a deep-reasoning model to a lighter one mid-session costs a cache invalidation; don't pay it unless the remaining session is long enough to amortize
...
At session start: scan for uncommitted changes or commits made outside your AI tool. Document anything found in `log.md` (or the session file for non-slice).
...
| **M** (medium) | Single session, 2–6 tasks, isolated scope, few real design choices                                                    | Lightweight: inline live progress tracker + 1-paragraph "Plan" at top of `log.md`. No separate planning session. |
...
Plans specify **decisions and sequencing**, not boilerplate code. Your AI tool at execution time has full context of the current codebase, reads the affected files, and writes code that matches local patterns. Over-specified plans lock in assumptions that may not match reality AND waste tokens twice (authoring + re-processing).
...
- Full test bodies for obvious assertions (say what's being tested; the AI tool writes at execution with full context)
- Complete implementations that follow a pattern already shown once in the plan

**Rule of thumb:** if you're pre-writing code that the AI tool could produce in 30 seconds by reading the target file, don't. Describe the transformation, give one canonical example, trust execution-time judgment for the rest.
...
Every Implementation or Closing session with 2+ tasks uses a **live progress tracker** for visual status + a **scrollback-readable markdown table** in STOP messages. Two persistence layers, same story. The tracker mechanism is tool-specific — see your tool's overlay (Claude Code → `TodoWrite`; Codex → its equivalent or inline markdown checklist in `log.md`).

Rules:

1. **Session start:** populate the tracker from the active sub-slice's `plan.md` — one entry per Level 3 task (plus any amendments mid-slice). Statuses seeded from `plan.md` checkboxes.
...
5. **At every STOP** (per-task): also print a compact markdown progress table in the conversation — same content, scrollback-readable. Format:
  ```
   ...
   |---|------|--------|--------|
  ```
6. **Session end:** the final tracker state is the resume point for the next session. `CHECKPOINT.md` snapshots the same state in durable form.

Why both a live tracker and the markdown table: the tracker is live UI (ephemeral to the session surface). The table in STOP messages is scrollback-readable across session history.
...
- **Parallel dispatch requires approval.** Delegating bounded, independent tasks to parallel workers (subagents, separate sessions, worker processes) can amortize cost — but independent research/exploration only. Never self-decide to parallelize implementation.
- **PR at Level 2 (sub-slice), not Level 3 (task).** All of a sub-slice's tasks accumulate to one PR.
- **Model routing:** Follow § Abstract roles → Stage → role routing. Upgrade (stage up to deeper-reasoning) the moment you notice you're making a tradeoff that matters or holding >3 independent concerns in mind. Tool overlays bind each role to a specific model — that binding is the only thing to edit when models change.
...
Budget is percentage-based — thresholds apply regardless of which model/window you're using. Consult your tool's overlay for the "how to check current usage" command.
...
The absolute-number thresholds per model/window are in your tool's overlay.
...
The AI tool should surface this; Yesid doesn't need to ask.
...
4. Append any OS-specific command fixes discovered to `<cloud>/workflow-knowledge/os-quirks/<os>.md`.
...
├── AGENTS.md        # tool-agnostic workflow contract (this file)
├── CLAUDE.md        # Claude Code entry point (thin pointer to AGENTS.md + Claude overlay)
├── .mcp.json        # MCP server definitions (tool-agnostic; empty by default)
├── .claude/         # Claude Code config (Claude-specific; settings.json + subagents + hooks)
├── .codex/          # Codex config (Codex-specific; create when needed)
└── package.json
```
...
- `docs/reference/tools/` — per-tool overlays (claude-code.md, codex.md)
...
- Hardcode tool-specific mechanics (model names, slash commands, tracker names) into this file — they go in the overlays under `docs/reference/tools/`
