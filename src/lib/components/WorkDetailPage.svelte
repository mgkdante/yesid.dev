...
<article class="mx-auto max-w-4xl px-4 pb-16 pt-8" data-testid="work-detail-page">
	...
	<!-- Hero section -->
	<header class="mb-8" use:reveal={{ direction: 'up', delay: 0 }}>
		<h1
			class="font-heading text-2xl font-bold leading-tight text-[var(--text-primary)] md:text-3xl lg:text-4xl"
		>
			{resolveLocale(project.title, 'en')}
		</h1>
		<p class="mt-2 text-sm text-[var(--text-muted)] md:text-base">
			{resolveLocale(project.oneLiner, 'en')}
		</p>
		<!-- Gradient divider, same pattern as BlogDetailHeader -->
		<div
			class="mt-5 h-px"
			style="background: linear-gradient(90deg, #E07800, #2a2a2a);"
		></div>
	</header>
	...
	<div class="flex flex-col gap-10 lg:flex-row">
		...
		<div class="min-w-0 flex-1">
			...
			<div class="mb-8" use:reveal={{ direction: 'up', delay: 100 }}>
				<p class="text-sm leading-relaxed text-[#ccc] md:text-base">
					{resolveLocale(project.description, 'en')}
				</p>
			</div>

			<!-- Project sections as cards -->
			{#each project.sections as section, i}
				<div
					class="mb-6 rounded-lg border border-[#2a2a2a] bg-[#141414] p-5"
					use:reveal={{ direction: 'up', delay: 150 + i * 80 }}
				>
					<h2 class="mb-2 font-heading text-lg font-bold text-[var(--text-primary)]">
			...
			{#if readmeHtml}
				<div
					class="mt-8 rounded-lg border border-[#2a2a2a] bg-[#141414] p-5"
