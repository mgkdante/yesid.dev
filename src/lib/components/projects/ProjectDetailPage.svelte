...
<script lang="ts">
	...
	import ProjectDetailSidebar from './ProjectDetailSidebar.svelte';
	...
	import { DetailHero } from '$lib/components/shells';
...
<article class="w-full pb-16 pt-8" data-testid="project-detail-page">
	...
	<div class="mx-auto px-[var(--space-page-x)]" style="max-width: var(--container-wide)">
		<DetailHero
			backHref="/projects"
			backLabel={resolveLocale(labels.backLink, 'en')}
			title={resolveLocale(project.title, 'en')}
			subtitle={resolveLocale(project.oneLiner, 'en')}
		>
			{#snippet extras()}
		...
		<div class="mb-8 lg:hidden">
			<ProjectDetailSidebar {project} {services} {serviceSvgContents} />
		</div>
	</div>
