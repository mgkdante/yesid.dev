<script lang="ts">
	...
	export interface SectionHeadingProps {
		/** Main heading text (dot appended automatically) */
		heading: string;
	...
	let { heading, subheading, level = 2, class: className, ...rest }: SectionHeadingProps = $props();
...
<div data-slot="section-heading" class={cn('', className)} {...rest}>
	<svelte:element this={tag} class="section-heading-text">
		{heading}<span data-slot="section-heading-dot" class="section-heading-dot">.</span>
	</svelte:element>
	{#if subheading}
