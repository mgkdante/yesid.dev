...
Architecture: Pure functions (zero DOM/framework dependency). Same engine works in browser, server route, or future API. Adding a tech = one markdown file (pre-Slice-18) or one Directus collection row (post-Slice-18) — the engine adapts automatically. CMS-ready via the Slice 17b service seam; **Directus is the chosen CMS** (Slice 18, Cloud Content Layer — pivoted from Payload 2026-04-22 after research in `slice-headless-cms-best-practices`; see `docs/slices/slice-18/README.md` for current direction).
...
- **CMS & Content** (Tier 2): Directus (primary offering — pivoted from Payload 2026-04-22), Keystatic (budget "Static" tier), Webflow CMS, WordPress
