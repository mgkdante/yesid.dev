...
| **18h** | Meta + route_seo (singleton + og_image) — first repo singleton; combined brand identity + SEO defaults per Q2 amendment; two-method API per Q9 (meta.site + meta.siteSeoDefaults sharing fetchSingletonRow memo); composedTitleStrategy flag preserves home em-dash format per P3 | ✅ closed 2026-04-27 | shipped (PR #82 → ab4343f) | [18h-meta-route-seo/](18h-meta-route-seo/) |
| **18h-ii** | Icons collection (centralized icon library, Iconify-backed) + apps/web /admin/icons Svelte browse page | ✅ closed 2026-04-27 | shipped (PR pending, 18 commits) | [18h-ii-icons/](18h-ii-icons/) |
...
| **D-AMEND-1** | **Amends D11.** Default = NO extensions, custom OR community. Each extension requires a written justification + a quality bar: **≥50 stars OR ≥1 year of maintenance OR org-backed** (e.g., `directus-extension-sync@tractr.net` qualifies — 451 stars, mature, org-backed). For each adopted extension: file a quarterly recheck issue + name an owner. **Background:** original (less strict) phrasing was "community-marketplace OK when listed + maintained" but the first adoption attempt (`simple-iconify-picker` for 18h-ii) failed the empirical bar — 0 stars, 4 months old, single maintainer, host constraint stuck at `^10.10.0`. Pivoted to in-stack solution (apps/web /admin/icons Svelte page) — see 18h-ii decisions.md Q5 + Q-AMEND-1. The amended bar would have rejected the picker upfront. | shape locked | 2026-04-27 (18h-ii pivot) |
