...
| 2026-04-24 | **D12 strict boundary → app independence convention.** Apps are NOT workspace packages (only `packages/*`). Cross-app imports require relative paths — caught in code review. Dedicated CI check (originally Task 15 scope) removed; YAGNI. `packages/shared` remains the only legitimate cross-app surface. | Natural separation + code review sufficient enforcement; avoids custom lint-rule maintenance burden. |
