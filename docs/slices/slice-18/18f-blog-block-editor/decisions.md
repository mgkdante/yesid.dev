...
| AM7 | **`probe-test-row` preserved** (deletion deferred per Phase 2 Task 8). Useful for re-probing edge cases (e.g., delimiter block shape, compound marks). Auto-cleanup during Phase 5 push if it conflicts; manual cleanup before slice close. Note: deleted live in Phase 5 Task 30 retry (constraint blocked NOT NULL category add); not preserved in the end. | Operational decision | Phase 2 Task 8 marked deferred-not-blocking. |

## Amendments (2026-04-26 — post-Phase 5 corrections)

| ID | Amendment | Source | Effect |
|---|---|---|---|
| AM2.5 | **Q2 reversed — blog drops translations junction; title + excerpt are flat fields on `blog_posts` parent.** Original Q2 (translations(title,excerpt) + lang on parent + body on parent) created a half-i18n disparity: body explicitly mono-language via `lang`, but title/excerpt translatable. Authors would never write title in one language while body in another. Editorial reality: each post is mono-language end-to-end. Drop `blog_posts_translations` collection; promote `title` (string, required) + `excerpt` (text, required, ≤500 chars) to `blog_posts` directly. CONVENTIONS § 8 ("every localized field on translations junction") is the default for content that's actually translated; blog posts are NOT — the `lang` field on parent is the i18n primitive. Verified: only blog has this disparity; projects + services correctly have NO `lang` field on parent + translations as the sole i18n primitive. | User 2026-04-26 | Schema delta in commit (this AM2.5): drop blog_posts_translations collection + 5 fields + 2 relations. Drop blog_posts.translations alias. Add blog_posts.title + blog_posts.excerpt. Drop 8 permissions rows (translations) + update Public blog_posts read fields list. Downstream impacts: BlogPost type → `title: string` + `excerpt: string` (was LocalizedString); Phase 7 fixture shape simplifies; Phase 8 seed simplifies; Phase 9 adapter `toBlogPost` direct field reads. Existing consumers (BlogRow.svelte, BlogDetailHeader.svelte) drop `resolveLocale(post.title, 'en')` calls — read `post.title` directly. |
| AM8 | **Block Editor UX (top-toolbar / Notion-style) deferred to 18j.** User flagged the `+` paradigm as friction (have to dig through menus for h1-h5/bold/italic). Stock Editor.js shows inline toolbar on text-select but block-level switching uses `+` or block tunes. Fixing requires custom Directus interface extension (same D11 blocker as AM3 + AM4). Deferred bundle: build `yesid-block-editor` extension with always-visible top toolbar + language picker (AM3) + embed plugin (AM4) + custom branding. ~1–2 sessions in 18j or post-18. File as GH issue at 18f close. | User 2026-04-26 | Phase 6 + Phase 10 unchanged for 18f — ship with stock Editor.js. Author guide deferred to 18j. |
