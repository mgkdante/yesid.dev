...
| 2026-04-18 | Review-driven post-ship amendments (F1–F5 from independent reviewer pass): `registry.jsonc` gained a `claude_equivalent` annotation per MCP (F4, reading-layer); `SKILL.md` rewritten tool-agnostic and bumped to `version: 1.1.0` (F1); 17k-10 claim narrowed to skill-only scope (F3); plugin apply codepath stress-tested on `playground@claude-plugins-official` (F5); handoff addenda recorded in §Task 17k-5, 17k-6, 17k-9, 17k-10 | An independent Claude Code reviewer pass surfaced overstated portability claims, unverified codepaths, and content-layer inconsistency between cloud source and installed targets. Honesty fixes shipped before close; full modular MCP system + `install.ts` rewrite deferred to workflow-repo Slice 1 |
| 2026-04-18 | Acceptance criterion "`<cloud>/workflow-knowledge/stack/skills/workflow-efficiency/SKILL.md` is a verbatim copy of `~/.claude/skills/workflow-efficiency/SKILL.md`" amended to "semantically equivalent modulo (a) registry-injected `version:` frontmatter per D021 and (b) the post-ship tool-agnostic rewrite shipped in §Task 17k-6's addendum" | The original criterion predated D021 (install-time version injection) and review finding F1 (skill body was Claude-only and needed tool-agnostic content). Both are deliberate, both make the mirror content-semantically correct but byte-different |
| 2026-04-18 | Task 17k-8 (cross-tool Codex verification) deferred | Synthetic "Codex summarize `AGENTS.md`" exercise produces weaker signal than real cross-tool usage. The workflow is about to migrate to its own GitHub repo (see row below) and get cloned onto the `transit` project (different field: Neon → Oracle VPS Postgres port). That real migration + bootstrap IS the cross-tool verification; a synthetic 17k-8 now delays it by hours without adding information. Acceptance criterion "A Codex session opened in the repo confirms it can read `AGENTS.md` + overlay and explain the workflow" is satisfied in spirit by scheduled workflow-repo Slice 1 + transit bootstrap, not by an in-slice task |
| 2026-04-18 | Planned "17l" (yesid.dev sub-slice for modular MCP system + registry routing + workflow-repo migration) **cancelled**; all that work moves to Slice 1 of a new `workflow` project at `C:\Users\otalo\Yesito\Projects\workflow` (separate GitHub repo, private) | Architectural reframe: the workflow itself is Yesid's personal framework / product / IP — not a yesid.dev sub-slice. It deserves its own repo with its own slice hierarchy, slice counts starting from 1. That new repo extracts the portable cross-tool contract (AGENTS.md base + per-tool overlays + slice templates + stack registry + install.ts + mode presets + `<cloud>/claude-config/` + `<cloud>/codex-config/` moving in-repo) so it can be `git clone`-ed onto any project Yesid works on. yesid.dev's Slice 17 closes cleanly at 17k; there will be no 17l |
